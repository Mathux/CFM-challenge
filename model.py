...
from keras.layers import Dense, Dropout, Embedding, LSTM, Conv1D, PReLU, 
...
class LSTMModel(object) :
    ...
    def __init__(self,X, y, eqt_embeddings_size = 80, lstm_out_dim = 5, 
        ...
        self.returns_length = len(return_cols)
    ...
    def create_model(self):
        ...
        eqt_emb = Embedding(output_dim = self.eqt_embeddings_size, 
                                   input_dim = 1, input_length = self.n_eqt, 
                                   name = 'eqt embeddings')(eqt_code_input)
        eqt_emb = SpatialDropout1D(self.dropout_rate)(eqt_emb)

        
        # Then the LSTM/CNN1D for the returns time series 
        returns_input = Input(shape = (self.return_length,1),
                              name = 'returns_input')
        ...
        vol_input = Input(shape = (self.return_length,1),name = 'vol_input')
