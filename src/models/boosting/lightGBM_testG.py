...
def cond(col):
    if col.endswith(':00'):
        return True
    elif col in ["avg_return_date_eqt", "difference_to_market", "return_trend", "avg_market_return_eqt", "var_market_return_eqt", "countd_product", "avg_market_return_date"]:
        return True
    else:
        return False
...
not_return_cols = [col for col in data.train.data.columns if cond(col)]
data.train.data = data.train.data[not_return_cols]

not_return_cols = [col for col in data.val.data.columns if cond(col)]
data.val.data = data.val.data[not_return_cols]


test_id = data.test.data['ID']
data.test.data = data.test.data.drop(['ID','date'],axis = 1)
not_return_cols = [col for col in data.test.data.columns if cond(col)]
...
print("submission at testG.csv")
submission(preds, test_id, name="testG.csv")
