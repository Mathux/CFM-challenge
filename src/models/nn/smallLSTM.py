...
class smallLSTM(GeneralLSTM):
    ...
    def create_model(self):
        ...
        x = concatenate(
            [returns_input, market_returns_input, return_diff_to_market_input])

        temporal_features = JANET(
            self.lstm_out_dim,
            return_sequences=False,
            dropout=self.dropout_lstm,
            recurrent_dropout=self.dropout_lstm_rec,
        ...
        temporal_features = concatenate(
            [returns_features_lstm, temporal_features])

        temporal_features = Dense(64, activation='relu')(temporal_features)

        x = concatenate(
            [returns_features_lstm, context_eqt_day, temporal_features])
