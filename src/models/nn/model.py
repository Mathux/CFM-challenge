    ...
    def process_data(self, data, labels=None):
        ...
        def create_input(name, data, eps = 10**-10):
            if name == "eqt_code_input":
                ...
                temp_market_returns = data.groupby('date')[self.return_cols].mean()
                data.set_index(['date'], inplace=True)
                self.market_return_cols = ['market_returns_' + c for c in self.return_cols]
                data[self.market_return_cols] = temp_market_returns
                data.reset_index(inplace = True)
                temp_market_returns = data[self.market_return_cols].values
                temp_market_returns = temp_market_returns.reshape((temp_market_returns.shape[0],
                ...
                temp_eqt_returns = data.groupby('eqt_code')[self.return_cols].mean()
                data.set_index(['eqt_code'], inplace=True)
                self.eqt_return_cols = ['eqt_returns_' + c for c in self.return_cols]
                data[self.eqt_return_cols] = temp_eqt_returns
                data.reset_index(inplace = True)
                temp_eqt_returns = data[self.eqt_return_cols].values
                temp_eqt_returns = temp_eqt_returns.reshape((temp_eqt_returns.shape[0],
                                                            temp_eqt_returns.shape[1], 1))
                data = data.drop(self.eqt_return_cols,axis = 1)
                return temp_eqt_returns
            
            elif name == 'market_kurt_returns' :
                temp_kurt_returns = data.groupby('date')[self.return_cols].apply(pd.DataFrame.kurt)
                data.set_index(['date'], inplace=True)
                self.kurt_return_cols = ['kurt_returns_' + c for c in self.return_cols]
                data[self.kurt_return_cols] = temp_kurt_returns
                data.reset_index(inplace = True)
                temp_kurt_returns = data[self.kurt_return_cols].values
                temp_kurt_returns = temp_kurt_returns.reshape((temp_kurt_returns.shape[0],
                                                     temp_kurt_returns.shape[1], 1))
                data = data.drop(self.kurt_return_cols,axis = 1)
                return temp_kurt_returns
            
            elif name == 'eqt_kurt_returns' :
                temp_eqt_kurt_returns = data.groupby('eqt_code')[self.return_cols].apply(pd.DataFrame.kurt)
                data.set_index(['eqt_code'], inplace=True)
                self.eqt_kurt_return_cols = ['eqt_kurt_returns_' + c for c in self.return_cols]
                data[self.eqt_kurt_return_cols] = temp_eqt_kurt_returns
                data.reset_index(inplace = True)
                temp_eqt_kurt_returns = data[self.eqt_kurt_return_cols].values
                temp_eqt_kurt_returns = temp_eqt_kurt_returns.reshape((temp_eqt_kurt_returns.shape[0],
                ...
                return create_input('returns_input',data) - create_input('market_returns_input',data)
                ...
                temp_market_log_vol = (data.apply(lambda x : np.log(np.abs(x)+eps)))[self.return_cols]
                temp_df = pd.DataFrame()
                temp_df['date'] = data['date']
                temp_df[self.return_cols] = temp_market_log_vol
                temp_market_log_vol = temp_df.groupby('date')[self.return_cols].mean()
                temp_df.set_index(['date'], inplace=True)
                self.market_log_vol_cols = ['market_log_vol_' + c for c in self.return_cols]
                temp_df[self.market_log_vol_cols] = temp_market_log_vol
                temp_df.reset_index(inplace = True)
                temp_market_log_vol = temp_df[self.market_log_vol_cols].values
                temp_market_log_vol = temp_market_log_vol.reshape((temp_market_log_vol.shape[0],
                ...
                return create_input('log_vol_input') - create_input('market_log_vol_input')
