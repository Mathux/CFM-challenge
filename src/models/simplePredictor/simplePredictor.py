...
class SimplePredictor(object):
    def __init__(self,
        ...
        for code in eqt_code:
            ...
            data_code = data_code.drop(['eqt_code', 'end_of_day_return', 'ID'],
                                       axis=1)
            ...
            if np.sum(labels == 1) > 0 and np.sum(labels == 0) > 0:
                ...
                if classifier == 'LogisticRegression':
                    ...
                    self.predictors[code] = LogisticRegression(
                        solver='lbfgs', max_iter=500, C=self.C,
                        penalty='l2').fit(data_code, labels)

                elif classifier == 'SVM':

                    self.predictors[code] = SVC(
                        C=self.C, gamma='scale').fit(data_code, labels)

                elif classifier == 'KNN':

                    self.predictors[code] = KNeighborsClassifier(
                        n_neighbors=self.C).fit(data_code, labels)
                ...
                if np.sum(labels == 1) > 0:
                    self.predictors[code] = DummyClassifier(
                        strategy='constant', constant=1).fit(
                            data_code, labels)
                else:
                    self.predictors[code] = DummyClassifier(
                        strategy='constant', constant=0).fit(
                            data_code, labels)
    ...
    def score(self, X, y):
        ...
        for code in eqt_code:
            ...
            data_code = X[X['eqt_code'] == code].sort_values(by='ID')
            labels = data_code['end_of_day_return']
            data_code = data_code.drop(['end_of_day_return', 'ID', 'eqt_code'],
                                       axis=1)
            self.accuracy[code] = self.predictors[code].score(
                data_code, labels)
            stocks_done += 1
