...
class Stacking(object):
    def __init__(self,
        ...
        for j in range(len(self.weak_classifiers)):
            ...
            self.weak_classifiers[j].fit(train,
                                         train_labels['end_of_day_return'])
            new_dataset[j] = self.weak_classifiers[j].predict_proba(
                train)[:, 0]
    ...
    def __init__(self,
        ...
        if len(weights) == 0:
            self.weights = np.ones(
                len(base_classifiers)) / len(base_classifiers)
        else:
        ...
        for j in range(len(self.base_classifiers)):
            ...
            self.base_classifiers[j].fit(train,
                                         train_labels['end_of_day_return'])
    ...
    def predict(self, X):
        ...
        for j in range(len(self.base_classifiers)):
            predicted_proba.append(
                self.base_classifiers[j].predict_proba(X)[:, 0])
